...
{ // 在线性表L中下标为p的位置插入数据元素x，若下标p非法或线性表已满无法插入数据，返回0；插入成功返回值为1
  ...
  int i;
  if (isFullList_seq(L) == 1) //是否满表
  {
    printf("list is full");
    return 0;
  }
  if (p > L->curNum || p < 0) //是否合法
  {
...
int destroyList_seq(PseqList L)
{
  ...
  int temp; //返回值为销毁的线性表中现有数据元素的个数，若待销毁的线性表不存在，则返回0
  if (L)
  {
    temp = L->curNum;
    free(L->element);
...
{ //移除线性表中的所有重复元素；不要使用额外的数组空间，必须在原地修改输入数组 并在使用 O(1) 额外空间的条件下完成
  ...
  for (int i = 0; i < L->curNum; i++)
    ...
    {
      for (int k = 0; k < L->curNum; k++) //相邻删除
        if (L->element[i] == L->element[j])
        {
          flag = L->element[i];
          temp = delete_seq(L, flag);
        }
    }
  for (int i = 0; i < L->curNum; i++) //再次校验
    if (L->element[i] == flag)
      temp = delete_seq(L, flag);
}
